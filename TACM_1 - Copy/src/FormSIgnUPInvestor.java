 ...
 * @author Acer
 */
public class FormSIgnUPInvestor extends javax.swing.JFrame {

    /**
     * Creates new form FormSignUp1
     */
    public FormSIgnUPInvestor() {
        initComponents();
    }
...
String id = txtid.getText();
    String companyName = txtnama.getText();
        ...
        try (Statement stmt = conek1.GetConnection().createStatement()) {
            // Masukkan data ke tabel perusahaan
            String queryLogin = "INSERT INTO login (username, user_password, status) VALUES ('"
                    + username + "', '" + password + "', 'Investor')";
            stmt.executeUpdate(queryLogin);
            
            String queryCompany = "INSERT INTO investor (id_investor, username, nama, alamat, email,"
                    + " no_telp) VALUES ('" + id + "','"
                +username + "', '"    + companyName + "', '" + address + "', '" + email + "', '" + phone 
                    + "')";
            stmt.executeUpdate(queryCompany);

            // Ambil ID perusahaan yang baru saja dibuat
            ResultSet rs = stmt.executeQuery("SELECT LAST_INSERT_ID() AS id_investor");
            
            if (rs.next()) {
                String idInves = rs.getString("id_investor");
            

            // Masukkan data ke tabel login
          

            JOptionPane.showMessageDialog(null, "Akun investor berhasil dibuat! ID Investor: " + idInves);
            }
            new LoginUtama().setVisible(true);
            this.dispose();
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Terjadi kesalahan saat membuat akun investor.");
        }
...
// TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FormSIgnUPInvestor().setVisible(true);
            }
        });
    }
