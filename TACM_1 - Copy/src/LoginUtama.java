...
try {
    ...
    // Query untuk mengecek login dan data perusahaan
    String query = """
        SELECT l.username, l.user_password, l.status, p.id_perusahaan, i.id_investor
        FROM login l
        LEFT JOIN perusahaan p ON l.username = p.username
                   LEFT JOIN investor i ON l.username = i.username
    ...
    if (res.next()) {
        ...
        String userIdP = res.getString("id_perusahaan");
        String userIdi = res.getString("id_investor");
       
       
        
        if (password.equals(storedPassword)) {
            if (status.equalsIgnoreCase("Company") || status.equalsIgnoreCase("Investor")) {
                JOptionPane.showMessageDialog(null, "Login berhasil sebagai " + status);
                if (status.equalsIgnoreCase("Company")) {
                    showCompanyMenu(username,  userIdP,userIdi);
                } else {
                    showInvestorMenu(username, userIdi);
                }
            } else {
                JOptionPane.showMessageDialog(null, "Login gagal. Status tidak cocok.");
            }
        } else {
...
statement.close();
    ...
    e.printStackTrace();
    JOptionPane.showMessageDialog(null, "Terjadi kesalahan dengan koneksi database.");
}
 ...
 String pass =txtpassword.getText();
    ...
    private void showCompanyMenu(String username, String userIdP, String userIdi) {
        Menu_Perusahaan mp = new Menu_Perusahaan(username, userIdi, userIdP);
         mp.setVisible(true);
        this.dispose();
    }

    private void showInvestorMenu(String username, String userIdi) {
        Menu_Investor mi = new Menu_Investor(username, userIdi); 
          mi.setVisible(true);
          this.dispose();
